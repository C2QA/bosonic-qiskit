...
def test_plot_zero(capsys):
    with capsys.disabled():
        ...
        bosonic_qiskit.wigner.plot_wigner(circuit, trace, file="tests/zero.png", trace=False)
...
def test_plot_one(capsys):
    with capsys.disabled():
        ...
        # print(state)
        bosonic_qiskit.wigner.plot_wigner(circuit, state, file="tests/one.png")
...
def test_plot_wigner_projection(capsys):
    with capsys.disabled():
        ...
        bosonic_qiskit.wigner.plot_wigner_projection(
...
def test_cat_state_wigner_plot(capsys):
    with capsys.disabled():
        ...
        # qmr = bosonic_qiskit.QumodeRegister(num_qumodes=1, num_qubits_per_qumode=num_qubits_per_qumode)
        # qr = qiskit.QuantumRegister(size=1)
        # cr = qiskit.ClassicalRegister(size=1)
        # circuit = bosonic_qiskit.CVCircuit(qmr, qr, cr)
        ...
        # bosonic_qiskit.wigner.plot_wigner_projection(circuit, qr[0], file=wigner_filename)
        # assert Path(wigner_filename).is_file()
...
def test_wigner_mle(capsys):
    with capsys.disabled():
        ...
        wigner = bosonic_qiskit.wigner.wigner_mle(state)
...
def test_plot_wigner_snapshot(capsys):
    with capsys.disabled():
        ...
        bosonic_qiskit.wigner.plot_wigner_snapshot(circuit, result, "tests")
...
def test_plot_zero_contour(capsys):
    with capsys.disabled():
        ...
        bosonic_qiskit.wigner.plot(data, file=filename)
